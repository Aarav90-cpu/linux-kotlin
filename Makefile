...
BINDGEN		= bindgen
KOTLIN_DIR := $(srctree)/.kotlin-native
ifneq ($(wildcard $(KOTLIN_DIR)/bin/kotlinc-native),)
KOTLINC		= $(KOTLIN_DIR)/bin/kotlinc-native
KOTLIN_CINTEROP	= $(KOTLIN_DIR)/bin/cinterop
else
...
export KOTLINC KOTLIN_CINTEROP
export JAVAC JAVA GRADLE
...
ifdef CONFIG_KOTLIN
	+$(Q)$(CONFIG_SHELL) $(srctree)/scripts/kotlin_is_available.sh
	$(Q)$(MAKE) $(build)=kotlin
endif
ifdef CONFIG_JAVA
	+$(Q)$(CONFIG_SHELL) $(srctree)/scripts/java_is_available.sh
endif
ifdef CONFIG_GRADLE
	+$(Q)$(CONFIG_SHELL) $(srctree)/scripts/gradle_is_available.sh
endif
ifdef CONFIG_ANDROID_SDK
	+$(Q)$(CONFIG_SHELL) $(srctree)/scripts/android_sdk_is_available.sh
endif
...
CLEAN_FILES += vmlinux.symvers modules-only.symvers \
	       ...
	       compile_commands.json rust/test kotlin/test \
...
help:
	...
	@echo  '  kotlinavailable - Checks whether the Kotlin/Native toolchain is'
	@echo  '                    available and, if not, explains why.'
	@echo  '  dir/file.[os]   - Build specified target only'
	@echo  ''
	@echo  'Java / Android targets:'
	@echo  '  javaavailable   - Checks whether the Java toolchain is available.'
	@echo  '  gradleavailable - Checks whether Gradle is available.'
	@echo  '  androidavailable- Checks whether the Android SDK is available.'
	@echo  ''
	@$(if $(dtstree), \
		echo 'Devicetree:'; \
...
# "Is Kotlin/Native available?" target
PHONY += kotlinavailable
kotlinavailable:
	+$(Q)$(CONFIG_SHELL) $(srctree)/scripts/kotlin_is_available.sh && echo "Kotlin/Native is available!"

# "Is Java available?" target
PHONY += javaavailable
javaavailable:
	+$(Q)$(CONFIG_SHELL) $(srctree)/scripts/java_is_available.sh && echo "Java is available!"

# "Is Gradle available?" target
PHONY += gradleavailable
gradleavailable:
	+$(Q)$(CONFIG_SHELL) $(srctree)/scripts/gradle_is_available.sh && echo "Gradle is available!"

# "Is Android SDK available?" target
PHONY += androidavailable
androidavailable:
	+$(Q)$(CONFIG_SHELL) $(srctree)/scripts/android_sdk_is_available.sh && echo "Android SDK is available!"
